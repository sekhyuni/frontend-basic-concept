...
- 렌더링 최적화
    - 최대한 reflow와 repaint를 줄이는 방향으로 최적화
        - reflow 횟수 최소화
            ...
            - 스타일 변경과 레이아웃 속성 조회가 반복될 때 발생하는 강제 동기 레이아웃 현상을 막기 위해 레이아웃 속성 조회를 먼저 수행하고 그다음 쓰기 로직을 수행하는 방식을 채택할 수 있음 (레이아웃 속성 조회 시 상황에 따라 reflow가 발생할 수 있지만 연속된 읽기는 캐시된 레이아웃 정보를 재사용하므로 최대 1회로 억제할 수 있으며, DOM 변경은 이 캐시를 무효화시켜 매번 새로운 reflow를 유발하므로 읽기를 먼저 일괄 처리한 후 쓰기를 일괄 처리하는 것이 효율적임)
...
||저장 가능한 크기|수명|저장 장소|접근 가능한 곳|서버로 HTTP 요청 시, 자동 전송 여부|
|:---:|:---:|:---:|:---:|:---:|:---:|
...
|Cookies|개당 4KB, 도메인당 최대 50~180개 (브라우저마다 상이, Chrome: 180개)|Session Cookies (Expires/Max-Age 미설정): 브라우저가 종료될 때까지, Persistent Cookies (Expires/Max-Age 설정): 만료 시간까지|Disk/Browser Memory|Browser/Server|O|
- 저장 장소에 대한 부연 설명
...
1. Event Bubbling
    - 특정 요소에서 Event가 발생했을 때, 해당 Event가 타겟 요소부터 최상위 요소인 html 요소까지 점점 더 상위 요소들로 전파되어가는 특성
    - 대표적으로 Bubbling되는 Event: click, mousedown, mouseup, wheel, scroll, keydown, keyup
        ```html
        <html>
            <head>
                <style>
        ...
        ```
    - Event Bubbling을 활용한 예
        - Event Delegation
            - Event가 Bubbling되는 특성을 활용하여 Event Handling이 필요한 하위 요소들의 상위 요소가 하위 요소들의 Event를 처리하는 패턴
                ```html
                <div>
                ...
                ```
            - 구현 순서
                1. container에 하나의 Event Handler를 할당
                1. Event Handler에서 event.target을 사용하여 Event가 발생한 요소가 어디인지 알아냄
                1. 원하는 요소에서 Event가 발생했다고 확인되면 Event를 Handling
            - 장점
                1. Event Handler를 하나만 할당해도 되므로 메모리 사용량을 줄일 수 있음
                1. 동적으로 노드 추가 시 매번 Event Handler를 할당하지 않아도 되므로 코드 복잡도를 줄일 수 있음
            - 단점
                1. Event Delegation을 사용하려면 Event가 반드시 Bubbling되어야 하는데, 몇몇 Event는 Bubbling되지 않음
1. Event Capturing
    - 특정 요소에서 Event가 발생했을 때, 해당 Event가 최상위 요소인 html 요소부터 타겟 요소까지 점점 더 하위 요소들로 전달되어가는 특성
        ```html
        <html>
            <head>
                <style>
...
1. Event 전파 막기
    ```javascript
    event.stopPropagation(); // Bubbling 또는 Capturing 전파를 막고 싶을 때 사용 (예: 모달 콘텐츠 클릭 시 배경 클릭 Event 차단)
    event.stopImmediatePropagation(); // Bubbling 또는 Capturing 전파뿐만 아니라, 현재 실행 중인 Event Handler 이후 어떤 Event Handler도 실행시키고 싶지 않을 때 사용
    ```
